...
format_diff() {
	...
	if [[ $CLANG_FORMAT_MAJOR_VERSION -gt "9" ]]; then
		local_format="$(docker run -i -v "$(pwd)":"$(pwd)" -w "$(pwd)" --rm ghcr.io/jidicula/clang-format:"$CLANG_FORMAT_MAJOR_VERSION" -n --Werror --style=file:"./.clang-format-for-format-sh" "${filepath}")"
