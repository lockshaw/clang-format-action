...
BASE_URL="https://github.com/muttleyxd/clang-tools-static-binaries/releases/download/master-f4f85437/clang-format-16_linux-amd64"

CLANG_FORMAT_CONFIG="$GIT_ROOT/.clang-format-for-format-sh"
mapfile -t FILES < <(git ls-files ':!:triton/**' '*.h' '*.cc' '*.cpp' '*.cu' '*.c' '*.cxx' '*.c++' '*.ino' '*.pde')
